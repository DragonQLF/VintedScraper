...
export default function Dashboard() {
  ...
  const getMatchStatus = (matchId: string): 'new' | 'price_drop' | 'other' | undefined => {
    ...
    if (matchNotifications.some(notif => notif.type === 'NEW_MATCH')) {
      return 'new';
    }
    if (matchNotifications.some(notif => notif.type === 'PRICE_DROP')) {
      return 'price_drop';
    }
    return undefined;
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4 p-4 bg-indigo-50 dark:bg-gray-800 rounded-xl shadow">
        <div>
          <h1 className="text-2xl font-bold text-indigo-900 dark:text-indigo-100">Dashboard</h1>
          <p className="text-sm text-gray-600 dark:text-gray-300">Welcome back! Here's a summary of your Vinted matches.</p>
        </div>
        <div className="flex gap-6">
          <div className="flex flex-col items-center">
            <span className="text-lg font-semibold text-indigo-700 dark:text-indigo-200">{totalMatches}</span>
            <span className="text-xs text-gray-500 dark:text-gray-300">Total Matches</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-lg font-semibold text-green-600 dark:text-green-400">{newMatchesCount}</span>
            <span className="text-xs text-gray-500 dark:text-gray-300">New</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-lg font-semibold text-blue-600 dark:text-blue-400">{priceDropsCount}</span>
            <span className="text-xs text-gray-500 dark:text-gray-300">Price Drops</span>
          </div>
        </div>
      </div>
      <ScraperStatus />
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900">Search Profiles</h2>
          <button
            onClick={() => handleEditProfile(null)}
            className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:bg-indigo-700 dark:hover:bg-indigo-800 dark:text-white"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {profiles.map(profile => (
            <div key={profile.id} className="bg-white dark:bg-gray-800 p-4 rounded-md shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-lg font-medium text-gray-800 dark:text-gray-100">{profile.name}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">{profile._count?.matches || 0} matches</p>
                  {profile.keywords && (
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Keywords: {profile.keywords}</p>
                  )}
                  {profile.category && (
                    <p className="text-sm text-gray-500 dark:text-gray-400">Category: {profile.category}</p>
                  )}
                  {profile.minPrice && profile.maxPrice && (
                    <p className="text-sm text-gray-500 dark:text-gray-400">Price: €{profile.minPrice} - €{profile.maxPrice}</p>
                  )}
                </div>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => handleEditProfile(profile)}
                    className="p-1 rounded-full text-gray-400 hover:text-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                    title="Edit Profile"
                  >
                  ...
                  <button
                    ...
                    className="p-1 rounded-full text-gray-400 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50"
                    title="Delete Profile"
                  >
                    {deleteProfile.isPending && deleteProfile.variables === profile.id ? (
                      <ArrowPathIcon className="animate-spin h-5 w-5 text-red-600" aria-hidden="true" />
                    ) : (
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div className="flex items-center space-x-4">
            <select
              ...
              className="block w-full rounded-md border-gray-300 dark:border-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            >
              <option value="all">All Profiles</option>
              {profiles.map(profile => (
                <option key={profile.id} value={profile.id}>
                  {profile.name} ({profile._count?.matches || 0})
                </option>
              ))}
            </select>
            <select
                  ...
                  {data.matches.map(match => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <a href={match.productUrl} target="_blank" rel="noopener noreferrer" className="block">
                          <img
                            src={match.imageUrls}
                            alt={match.title}
